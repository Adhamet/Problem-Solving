...
int32_t main() {
	...
	vector<vector<int>> mem((int)nums.size(), vector<int>(MXN, -1));
	auto go = [&](auto self, int i, int cur) -> int {
		if (cur == 0) return 1;
		if (i >= (int)nums.size() || cur < 0) return 0;

		int &ret = mem[i][cur];
		if (~ret) return ret;
		ret = 0;

		ret = self(self, i + 1, cur);
		if (nums[i] <= cur) ret = (ret + self(self, i, cur - nums[i])) % MOD;
		return ret;
	};
