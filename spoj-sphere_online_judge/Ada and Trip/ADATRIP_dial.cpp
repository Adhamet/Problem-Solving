...
signed main() {
	...
	auto dial = [&](int start) {
		...
		const int mod = 11;
		vector<queue<int>> buckets(mod);
		buckets[0].push(start);
		int currD = 0, mxD = 0;

		while (true) {
			while (currD <= mxD && buckets[currD % mod].empty()) currD += 1;
			if (currD > mxD) break;

			int v = buckets[currD % mod].front();
			buckets[currD % mod].pop();

			if (dist[v] != currD) continue;
			for (auto& [u, w]: g[v]) {
				if (dist[u] > dist[v] + w) {
					dist[u] = dist[v] + w;
					buckets[dist[u] % mod].push(u);
					mxD = max(mxD, dist[u]);
				}
			}
		}
